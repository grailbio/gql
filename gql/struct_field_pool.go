// Code generated by " ../../../../github.com/grailbio/base/gtl/generate.py --output=struct_field_pool.go --prefix=structField --PREFIX=structField --package=gql -DELEM=[]StructField ../../../../github.com/grailbio/base/gtl/randomized_freepool.go.tpl ". DO NOT EDIT.

// +build !race

// structFieldFreePool is thread-safe pool that uses power-of-two loadbalancing across
// CPUs.
...
// structFieldFreePool is a variation of sync.Pool, specialized for a concrete type.
//
...
type structFieldFreePool struct {
	...
	local        []structFieldPoolLocal
	maxLocalSize int64
}

const (
	structFieldMaxPrivateElems = 4
	structFieldCacheLineSize   = 64
)

type structFieldPoolLocalInner struct {
	private     [structFieldMaxPrivateElems][]StructField // Can be used only by the respective P.
	privateSize int

	shared     [][]StructField // Can be used by any P.
	sharedSize int64           // ==len(shared), but can be accessed w/o holding mu.
	mu         sync.Mutex      // Protects shared.
}

type structFieldPoolLocal struct {
	structFieldPoolLocalInner
	// Pad prevents false sharing.
	pad [structFieldCacheLineSize - unsafe.Sizeof(structFieldPoolLocalInner{})%structFieldCacheLineSize]byte
}

// NewstructFieldFreePool creates a new free object pool. new should create a new
...
func NewstructFieldFreePool(new func() []StructField, maxSize int) *structFieldFreePool {
	...
	p := &structFieldFreePool{
		new:          new,
		local:        make([]structFieldPoolLocal, maxProcs),
		maxLocalSize: int64(maxLocalSize),
	}
	return p
}

func (p *structFieldFreePool) pin() *structFieldPoolLocal {
	pid := runtime_procPin()
...
func (p *structFieldFreePool) Put(x []StructField) {
	...
	l := p.pin()
	if l.privateSize < structFieldMaxPrivateElems {
...
func (p *structFieldFreePool) ApproxLen() int {
	n := 0
	for i := range p.local {
