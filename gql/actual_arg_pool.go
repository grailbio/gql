// Code generated by " ../../../../github.com/grailbio/base/gtl/generate.py --output=actual_arg_pool.go --prefix=actualArg --PREFIX=actualArg --package=gql -DELEM=[]ActualArg ../../../../github.com/grailbio/base/gtl/randomized_freepool.go.tpl ". DO NOT EDIT.

// +build !race

// actualArgFreePool is thread-safe pool that uses power-of-two loadbalancing across
// CPUs.
...
// actualArgFreePool is a variation of sync.Pool, specialized for a concrete type.
//
...
type actualArgFreePool struct {
	new          func() []ActualArg
	local        []actualArgPoolLocal
	maxLocalSize int64
}

const (
	actualArgMaxPrivateElems = 4
	actualArgCacheLineSize   = 64
)

type actualArgPoolLocalInner struct {
	private     [actualArgMaxPrivateElems][]ActualArg // Can be used only by the respective P.
	privateSize int

	shared     [][]ActualArg // Can be used by any P.
	sharedSize int64         // ==len(shared), but can be accessed w/o holding mu.
	mu         sync.Mutex    // Protects shared.
}

type actualArgPoolLocal struct {
	actualArgPoolLocalInner
	// Pad prevents false sharing.
	pad [actualArgCacheLineSize - unsafe.Sizeof(actualArgPoolLocalInner{})%actualArgCacheLineSize]byte
}

// NewactualArgFreePool creates a new free object pool. new should create a new
...
func NewactualArgFreePool(new func() []ActualArg, maxSize int) *actualArgFreePool {
	...
	p := &actualArgFreePool{
		new:          new,
		local:        make([]actualArgPoolLocal, maxProcs),
		maxLocalSize: int64(maxLocalSize),
	}
	return p
}

func (p *actualArgFreePool) pin() *actualArgPoolLocal {
	pid := runtime_procPin()
...
func (p *actualArgFreePool) Put(x []ActualArg) {
	...
	l := p.pin()
	if l.privateSize < actualArgMaxPrivateElems {
...
func (p *actualArgFreePool) ApproxLen() int {
	n := 0
	for i := range p.local {
